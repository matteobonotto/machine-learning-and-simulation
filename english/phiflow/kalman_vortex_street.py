...
def main():
    ...
    # SPEED = vis.control(2.)
    SPEED = 5
    velocity = StaggeredGrid((SPEED, .1), ZERO_GRADIENT, x=128, y=64, bounds=Box(x=128, y=64))
    velocity_add = StaggeredGrid((0, .3), ZERO_GRADIENT, x=128, y=64, bounds=Box(x=128, y=64))
    CYLINDER = [
        Obstacle(geom.infinite_cylinder(x=15, y=32, radius=5, inf_dim=None)),
        Obstacle(geom.infinite_cylinder(x=35, y=25, radius=5, inf_dim=None)),
    ]
    ...
    for i in tqdm(range(N_TIME_STEPS)):
        ...
        trajectory.append(vorticity)

        vorticity_values_extracted = vorticity.values.numpy("y,x")
        plt.imshow(
            vorticity_values_extracted, 
            origin="lower",
            cmap = 'inferno',
